...
then
    ...
    then
        ...
        echo "UPDATE_XML_UNAVAILABLE ${update_xml_url} ${update_xml} ${update_xml_headers} ${update_xml_curl_exit_code}" > "$(mktemp -t failure.XXXXXXXXXX)"
    else
        ...
        echo "UPDATE_XML_REDIRECT_FAILED ${update_xml_url} ${update_xml_actual_url} ${update_xml} ${update_xml_headers} ${update_xml_curl_exit_code}" > "$(mktemp -t failure.XXXXXXXXXX)"
    fi
fi
