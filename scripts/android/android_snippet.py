...
def createEmptySnippets(aus_host, aus_user, aus_key, sd, snip):
    ...
    ssh(
        aus_host, 'touch %s %s' % (
            ' '.join(['%s/%s' % (sd, x) for x in snip]), sd),
        aus_user, aus_key)
...
def uploadSnippets(aus_host, aus_user, aus_key, aus_base,
    ...
    empty_snippet_directory = '%s/%s/%s/%s' % (
        aus_base, abi, new_buildid, locale)
    ...
    createAusUploadDirectory(
        aus_host, aus_user, aus_key, empty_snippet_directory)
    createEmptySnippets(
        aus_host, aus_user, aus_key, empty_snippet_directory, snippets)
