...
detailsUrl=http://details\n"""
        got = createSnippet(schema=1, type_='partial', url='http://bar', hash_='aaaaa', size=5,
...
platformVersion=1
detailsUrl=http://details\n"""
        got = createSnippet(schema=2, type_='complete', url='http://foo', hash_='xxxxx', size=123,
...
platformVersion=1
detailsUrl=http://details\n"""
        got = createSnippet(schema=2, type_='complete', url='http://foo', hash_='yyyyy', size=123,
    ...
    def testInvalidOptionalAttribute(self):
        self.assertRaises(SnippetError, createSnippet, 2, 1, 1, 1, 1, 1, 1, 1, 1, foo='bar')
...
class TestGetSnippetPaths(unittest.TestCase):
    def testOnePlatform(self):
        with mock.patch.dict(ftp_update_platform_map, {'p': ['pp']}):
            ...
            got = getSnippetPaths(product='foo', version='1', platform='p', buildid=2,
                locale='aa', channel='c', type_='complete')
            self.assertEquals(got, expected)

    def testMultiplePlatforms(self):
        with mock.patch.dict(ftp_update_platform_map, {'p': ['pp1', 'pp2']}):
            expected = ['foo/1/pp1/2/bb/c/partial.txt', 'foo/1/pp2/2/bb/c/partial.txt']
            got = getSnippetPaths(product='foo', version='1', platform='p', buildid=2,
                locale='bb', channel='c', type_='partial')
            self.assertEquals(got, expected)

    def testBadPlatform(self):
        self.assertRaises(SnippetError, getSnippetPaths, 'foo', 'foo', 'foo', 'foo', 1, 'foo', 'complete')
