...
if __name__ == '__main__':
    parser = optparse.OptionParser(
        'Usage: %prog [-F fiasco]|[--rootfs ubifile] -o outdir')
    parser.add_option('-F', '--fiasco', action='store', dest='fiasco',
                      help='Fiasco file to extract rootfs from')
    parser.add_option(
        '--flasher', action='store', dest='flasher', default='./flasher-3.5',
    ...
    parser.add_option(
        '-m', '--mtd', action='store', dest='mtd', default='/dev/mtd0',
    ...
    try:
        assert not os.path.exists(
            options.output), 'I will not overwrite an existing image'
        if options.fiasco:
            tmpdir, ubifile = extract_fiasco(
                options.fiasco, options.flasher, 'rootfs.jffs2')
            extract_ubifile(ubifile, options.output,
                            options.mtd, options.rsync, tmpdir=tmpdir)
        elif options.ubifile:
            extract_ubifile(
                options.fiasco, options.output, options.mtd, options.rsync)
    except AssertionError, ae:
        print 'ERROR: %s' % ae.args[0]
