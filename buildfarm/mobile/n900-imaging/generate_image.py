...
if __name__ == '__main__':
    ...
    parser.add_option(
        '--mkfs-ubifs', action='store', dest='mkfs_ubifs', default='./mkfs.ubifs',
        help='specify a non-default mkfs.ubifs')
    parser.add_option(
        '--ubinize', action='store', dest='ubinize', default='./ubinize',
    ...
    try:
        ...
        for ending in ['.jffs2', '.ubifs', '.ubi']:
            assert not options.output.endswith(
                ending), 'You should not use the "%" file ending' % ending
    ...
    try:
        ...
        generate_ubifile(
            options.rootdir, options.output, rsync, mkfs_ubifs, ubinize)
    except AssertionError, ae:
        print 'ERROR: %s' % ae.args[0]
